...
- Follow and execute the `Preconditions` in the "cold start" test above.

### Steps

1. Follow the instructions in de `README.md` under the section `Solid authentication` to save a secret and id.
2. Run `npm start` to start the synchronisation agent.
3. Wait a couple of seconds until the initial cold start of the synchronisation process is completed.
